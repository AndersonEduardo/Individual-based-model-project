...
class ambiente:
    def __init__(self,N,G,S,parN,parG,parS,K,Ncontatos,HospMuta,ParaMuta,tempoCriticoK): 
        ...
        self.numGen = G #len(self.vetorDeGen) #definindo o numero de genotipos no ambiente
        ...
        for i in range(self.numSp): #para cada especie...
            for j in range(self.numGen): #...e para cada um de seus genotipos...
                for k in range(self.numInd): #...crie 'numInd' organismos.
                    self.ind = organismos(i+1,j+1) #criando um organismos
                    ...
                    self.vetorDeGen.extend([j+1]) #armazenando informacao genetica da populacao
    ...
    def infeccao1(self):
    ##modela infeccao ativa gene-gene (ativa = infectado infecta todos nao infectados em seu campo)##
        rd.shuffle(self.populacao) #embaralhando o vetor de individuos
        self.NumeroDeContatos =int(self.proporcaoDeContatos*len(self.populacao))
        for i in range(len(self.populacao)): #para cada um dos organismos na populacao...
            if (self.populacao[i].statusVida == 1) and (self.populacao[i].parasitado == 1):
                ...
                for j in self.contatoInfeccioso:
                    if self.populacao[j].parasitado == []:
                        if self.populacao[j].Gen == self.populacao[i].parGen:
                            self.populacao[j].parasitado = 1
                        else:
                            self.moeda = rd.random()
                            if self.moeda < 0.1: #probabildiade de infeccao nao especialista
                                self.populacao[j].parasitado = 1
    ...
    def infeccao3(self):
    ##modela infeccao gene-gene, em que um individuo nao infectado interage (aleatoriamente) com outros em seu campo de contato## 
        rd.shuffle(self.populacao)#embaralhando o vetor de individuos
        self.NumeroDeContatos =int(self.proporcaoDeContatos*len(self.populacao))
        for i in range(len(self.populacao)): #para cada um dos organismos na populacao...
            if (self.populacao[i].statusVida == 1) and (self.populacao[i].parasitado == 0):
                ...
                for j in self.contatoInfeccioso:
                    if self.populacao[j].parasitado == 1:
                        if self.populacao[j].parGen == self.populacao[i].Gen:
                            self.populacao[i].parasitado = 1
                        else:
                            self.moeda = rd.random()
                            if self.moeda < 0.5: #probabildiade de infeccao nao especialista
                                self.populacao[i].parasitado = 1
    ...
    def reproduz0(self):
    ##hospedeiros se reproduzem independentemente do status parasitario e capacidade de suporte##
        rd.shuffle(self.populacao) #embaralhando o vetor de individuos
        for i in range(len(self.populacao)): #para cada um dos organismos na populacao...
                ...
                if self.sucesso <= 1: #...e se obtiver sucesso...
                    ...
                    if (self.populacao[i].sex != self.populacao[self.parceiroReprodutivo].sex) and (self.populacao[self.parceiroReprodutivo].prenhez == 0):                    
                        ...
                        if self.mutacao <= self.probMutaHosp: #0.001: #...se tiver sucesso...
                            ...
                            self.genotipoMutante = rd.randint(1,1000) #...o genotipo do parasita que sera recebido sofre mutacao...
                            if (self.genotipoMutante not in self.vetorDeGen) and (self.genotipoMutante != self.meuGenotipo) and (self.genotipoMutante > 0): #...compare o genotipo do parasita novo com o do residente e com os exitentes no ambiente...
                                self.ind.Gen = self.genotipoMutante #...se for uma mutacao unica, faca o genotipo da prole.
                        self.populacao.append(self.ind) #adicionando o novo individuo a populacao.
                        self.vetorDeGen.extend([self.ind.Gen])

    def reproduz1(self):
    ##hospedeiros infectados podem nao se reproduzir (razao: trade-off ativacao imunologica X reproducao)##
        rd.shuffle(self.populacao) #embaralhando o vetor de individuos
        for i in range(len(self.populacao)): #para cada um dos organismos na populacao...
            if (self.populacao[i].statusVida == 1) and (self.populacao[i].prenhez == 0): # and (self.populacao[i].idade <= 10):
...
#                self.funcaoMorte = self.numeroDeVivos*self.K**-1 #funcao da dependencia de densidade
                ...
                if self.populacao[i].parasitado == 1:
                    self.funcaoMorteVirulencia = self.funcaoMorte * (1 + self.populacao[i].parGen*1000**-1) #aumento pela virulencia corresponde ao numero do genPar
    ...
    def atualizaPop(self):
        ...
        #riqueza genetica na populacao de hospedeiros
...
class start:
    ...
    def roda(self):
        for t in range(self.tempo):
            ...
            self.minhaPop.morre1() #morre2(t) #LEMBRE-SE: 'morre2()' depende do tempo.
            self.minhaPop.infeccao1()
            self.minhaPop.reproduz1()
            self.minhaPop.atualizaPop()
